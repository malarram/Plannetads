...
class Controller_New extends Controller {
    ...
    public function action_index() {
        ...
        if (Core::config('advertisement.only_admin_post') == 1 AND (
                !Auth::instance()->logged_in() OR ( Auth::instance()->logged_in() AND Auth::instance()->get_user()->id_role != Model_Role::ROLE_ADMIN))) {
            $this->redirect('default');
        }
...
//        $this->template->scripts['footer'][] = 'js/components/marked.js';
//        $this->template->scripts['footer'][] = 'js/components/htmleditor.js';
//        $this->template->scripts['footer'][] = 'js/jquery.sceditor.bbcode.min.js';

        $this->template->scripts['footer'][] = 'js/jasny-bootstrap.min.js';
//        $this->template->scripts['footer'][] = URL::base().'cdn/sweetalert.min.js';
        $this->template->scripts['footer'][] = URL::base() . 'cdn/selectize.min.js';
        $this->template->scripts['footer'][] = URL::base() . 'cdn/ouibounce.min.js';
