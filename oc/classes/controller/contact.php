            ...
            if (captcha::check('contact')) {
                ...
                if (!core::akismet($name_from, $email_from, core::post('message'))) {
                    ...
                    if (Email::content(core::config('email.notify_email'), core::config('general.site_name'), $email_from, $name_from, 'contact-admin', $replace)) {
                        ...
                        $contact->contact_type = Model_Contact::TYPE_DIRECT;
                        if (Auth::instance()->logged_in())
                            $contact->conatct_user_id = Auth::instance()->get_user()->id_user;
                        $contact->contact_name = $name_from;
                        $contact->contact_email = $email_from;
                        $contact->contact_subject = core::post('subject');
                        $contact->contact_message = core::post('message');
                        ...
                        if (isset($_FILES['attachment']) && !empty($_FILES['attachment']['tmp_name'])) {
                            ...
                            if (
                                ...
                                Alert::set(Alert::ERROR, __('Attachment failure'));
                            else
                                $contact->contact_attachment = $attachment_name;
                        }
                        $contact->save();
                        Alert::set(Alert::SUCCESS, __('Your message has been sent'));
                        HTTP::redirect(Route::url('contact'));
                    } else {
                ...
                if (!core::akismet($name_from, $email_from, core::post('message'))) {
                    if (core::config('general.messaging')) {
                        ...
                        $ret = $user_to->email('user-profile-contact', array('[EMAIL.BODY]' => core::post('message'),
                            '[EMAIL.SENDER]' => $name_from,
                            '[EMAIL.SUBJECT]' => core::post('subject'),
                            '[EMAIL.FROM]' => $email_from), $email_from, $name_from);
                    }
