...
class Model_Ad extends ORM {
    ...
    */
    ...
    {
        if ($this->loaded() && $this->plans)
        {
            $html = '';
            $plans = json_decode($this->plans,TRUE);
            foreach ($plans as $k => $v):
                $notify_class = array('featured' => 'uk-badge-primary','premium' => 'uk-badge-success','sponsored' => 'uk-badge-warning','highlighted' => 'uk-badge-danger','bumpup' => 'uk-badge-notification');
                $html .= "<div class='uk-badge {$notify_class[$k]}'>".ucfirst($k)."-".Date::format($v, 'd M')."</div>";
            endforeach;
