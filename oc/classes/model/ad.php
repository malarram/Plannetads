...
class Model_Ad extends ORM {
    ...
    public function save_image($image) {
        ...
        if (
                !Upload::valid($image) OR ! Upload::not_empty($image) OR ! Upload::type($image, explode(',', core::config('image.allowed_formats'))) OR ! Upload::size($image, core::config('image.max_image_size') . 'M')) {
    ...
    public function fbcomments() {
        if ($this->loaded()) {
            if ($this->status == self::STATUS_PUBLISHED AND strlen(core::config('advertisement.fbcomments')) > 0) {
                return View::factory('pages/ad/fbcomments', array('fbcomments' => core::config('advertisement.fbcomments'),
    ...
    public function disqus() {
        if ($this->loaded()) {
            if ($this->status == self::STATUS_PUBLISHED AND strlen(core::config('advertisement.disqus')) > 0) {
                return View::factory('pages/disqus', array('disqus' => core::config('advertisement.disqus')))
                                ->render();
            }
        }
    ...
    public function to_feature($days = NULL) {
        ...
        if ($this->loaded()) {
            if (!is_numeric($days)) {
                $plans = Model_Order::get_plans('featured');
    ...
    public function to_premium($days = NULL) {
        ...
        if ($this->loaded()) {
            ...
            $this->premium = Date::unix2mysql(time() + ($days * 24 * 60 * 60));
            try {
                $this->save();
            } catch (Exception $e) {
                throw HTTP_Exception::factory(500, $e->getMessage());
    ...
    public function to_sponsored($days = NULL) {
        ...
        if ($this->loaded()) {
            ...
            $this->sponsored = Date::unix2mysql(time() + ($days * 24 * 60 * 60));
            try {
                $this->save();
            } catch (Exception $e) {
                throw HTTP_Exception::factory(500, $e->getMessage());
    ...
    public function to_highlighted($days = NULL) {
        ...
        if ($this->loaded()) {
            ...
            $this->highlighted = Date::unix2mysql(time() + ($days * 24 * 60 * 60));
            try {
                $this->save();
            } catch (Exception $e) {
                throw HTTP_Exception::factory(500, $e->getMessage());
    ...
    public function to_bumpup($days = NULL) {
        ...
        if ($this->loaded()) {
            ...
            $this->bumpup = Date::unix2mysql(time() + ($days * 24 * 60 * 60));
            try {
                $this->save();
            } catch (Exception $e) {
                throw HTTP_Exception::factory(500, $e->getMessage());
...
// append to $values new custom values
    ...
    public static function FormatYoutubeURL($text) {
         $text = preg_replace('~(?#!js YouTubeId Rev:20160125_1800)
                ...
                ~ix', 'http://www.youtube.com/watch?v=$1',$text);
