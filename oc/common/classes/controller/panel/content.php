...
class Controller_Panel_Content extends Auth_Controller {
    ...
    //list index conatct messages
    public function action_contact() {
        $title = __('Contact Message(s)');
        $this->template->scripts['footer'][] = 'js/oc-panel/crud/index.js';
        Breadcrumbs::add(Breadcrumb::factory()->set_title($title));
        $this->template->title = $title;

        $contacts = (new Model_Contact)->where('contact_type', '=', 'direct')->order_by('created_at', 'DESC')->find_all();

        $this->template->content = View::factory('oc-panel/pages/content/contact', array('contacts' => $contacts, 'title' => $title));
    }

    //view conatct message
    public function action_viewcontact() {
        $id = $this->request->param('id');
        $title = __('View Message');

        $contact = new Model_Contact($id);

        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Contact'))->set_url(Route::url('oc-panel', array('controller' => 'content', 'action' => 'contact'))));
        Breadcrumbs::add(Breadcrumb::factory()->set_title($title));

        if ($contact->loaded()) {
            $this->template->title = $title;
            $this->template->content = View::factory('oc-panel/pages/content/view_contact', array('contact' => $contact, 'title' => $title));
        } else {
            Alert::set(Alert::INFO, __('Failed to load contact message'));
            HTTP::redirect(Route::url('oc-panel', array('controller' => 'content', 'action' => 'contact')));
        }
    }
